...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	private void heapify(int where) {
		...
		//
		int numParent=where;
		int numChildl=where*2;
		int numChildr=numChildl+1;
		ticker.tick(3);
		...
		int smaller = numChildl;
		if(numChildr<=size && array[numChildl].getValue().compareTo(array[numChildr].getValue()) > 0) {
			smaller = numChildr;
			//compare left and right child and find a smaller one
			// compare the smaller child with the parent child and swap if parent is bigger
		}
		ticker.tick(2);
		
		if(array[smaller].getValue().compareTo(array[numParent].getValue()) < 0) {
			moveItem(smaller, numParent);
			heapify(smaller);
			ticker.tick(2);
		}
