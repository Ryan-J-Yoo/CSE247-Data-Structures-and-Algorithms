...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	while (!pq.isEmpty()) {
    		...
    		Iterable<Edge> successors = v.vertex.edgesFrom();
    		for (Iterator<Edge> it = successors.iterator(); it.hasNext();) {
    			Edge e = it.next();
    			Decreaser<VertexAndDist> u = handles.get(e.to);
    			int newDistance = v.distance + weights.get(e);
    			VertexAndDist s = u.getValue();
    			if(newDistance < s.distance) {
    				u.decrease(new VertexAndDist(s.vertex, newDistance));
    				parentEdges.put(s.vertex, e);
    			}
    		}
    	}
    }
    ...
    public LinkedList<Edge> returnPath(Vertex endVertex) {
    	...
    	for(Vertex v = endVertex; v != startVertex; v = parentEdges.get(v).from) {
